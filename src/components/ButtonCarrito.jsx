...
const ButtonCarrito = ({ item }) => {
  ...
  const check = (item) => {
    ...
    if (localStorage.getItem("shop")) {
      ...
      if (localArray.includes(item.toString()))
        localArray = localArray.filter(
          (element) => element != item.toString()
        ); //si ya esta en el localStorage, lo borro
  ...
  const checkName = (item) => {
    //si el item está en el localStorage, devuelve true y se cambia el boton de add por remove
    if (localStorage.getItem("shop").split(",").includes(item.toString()))
      setTitle(false);
    else setTitle(true);
  };
  ...
  return (
    <button
      onClick={() => {
        check(item), setTitle(!title);
      }}
      className={title ? "cardAddShop" : "cardRemoveShop"}
    >
