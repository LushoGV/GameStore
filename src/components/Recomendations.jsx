...
const Recomendations = ({ category, description, id }) => {
  ...
  const filterRecomendations = () => {
    let filterGenre =
      category == "ARPG" || category == "Action RPG" ? "MMORPG" : category; //error de la api
    ...
    let filters = data.filter((element) => {
      if (
        ...
        element.genre.toLowerCase() == filterGenre.toLowerCase()
      )
    ...
    if (filters.length > 4)
      return filters.slice(
        0,
        4
      ); //si los resultados filtrados son mayor a 4, se seleccionan 4
    else return filters;
  };
