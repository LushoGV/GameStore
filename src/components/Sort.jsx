...
const Sort = () => {
  ...
  const obj = [
    {
      ...
      content: [
        ["alphabetical", "release-date", "relevance"],
        ["all", "pc", "browser"],
        ["All", categoriesList].flat(),
      ],
    },
  ];

  const changeURL = (item) => {
    //cuando se selecciona una opcion de cualquier dropbox
    ...
    obj.map((element) => {
      if (element.content[0].includes(item)) {
        //si la opcion seleccionada se encuentra en el drop orden
        urlText = "?sort=";
        urlParam = location.search;
      } else if (element.content[1].includes(item)) {
        //si la opcion seleccionada se encuentra en el drop plataformas
        urlText = "&platform=";
        urlParam = location.pathname;
      } else {
        //si la opcion seleccionada se encuentra en el drop categorias
        urlText = "#filter=";
    ...
    if (urlParam == location.search) {
      //si la opcion seleccionada se encuentra en el drop orden

      if (urlParam != "") {
        //si ya habia un filtro antes de este tipo
        newUrlParam = urlParam.replace(
          urlParam.substring(urlParam.indexOf(urlText) + urlText.length),
          item
        ); //reemplazo el filtro de la url por el nuevo
        navigate(`${location.pathname}${newUrlParam}${location.hash}`); // y redirecciono a la nueva url
      } else {
        //si no habia filtro de este tipo, se agrega redirecciona a la url actual + el nuevo filtro
      ...
      if (platform != "") {
        ...
        navigate(
          `/${item.toLowerCase()}${location.pathname}${location.search}${
            location.hash
          }`
        ); //nuevaPlataforma+orden+genero
      }
    } else {
      if (urlParam != "") {
        newUrlParam = urlParam.replace(
          urlParam.substring(urlParam.indexOf(urlText) + urlText.length),
          item
        ); //urlParam.substring(posicion inicial del id + largo del id) = filtro viejo
  ...
  useEffect(() => {
    //cuando algun filtro en la url cambia, se actualiza el estado que contiene el objeto

    setSortSelect({
      sort: location.search
        ? location.search.slice(6, location.search.length)
        : "relevance", //si hay algun filtro, se recorta de la url, sino se pone uno por defecto
      platform: location.pathname
        ? location.pathname.slice(1, location.pathname.indexOf("/search"))
        : "all",
      genre: location.hash
        ? location.hash.slice(8, location.hash.length)
        : "All",
    });
