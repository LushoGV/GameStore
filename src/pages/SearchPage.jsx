...
const SearchPage = () => {
  ...
  let query = location.pathname.slice(
    location.pathname.indexOf("/search=") + 8,
    location.pathname.length
  ); //separo la busqueda de la url
  let filters = location.hash.slice(8, location.hash.length); //filtro el genero
  let sort =
    location.search.slice(6, location.search.length) != ""
      ? location.search.slice(6, location.search.length)
      : "relevance"; //si existe, filtro el orden, sino pongo por defecto
  let platform = location.pathname.slice(
    1,
    location.pathname.indexOf("/search")
  ); //filtro plataforma
  ...
  const testFilter = () => {
    let content = data.filter(
      (element) => element.title.toLocaleLowerCase().startsWith(query) == true
    ); //empiezan con el query
    let moreContent = data.filter((element) => {
      if (
    ...
    if (filters == "" && platform == "")
      contentFiltered = content; //si no hay filtros, va todo
    else {
      if (platform != "" && platform != "all")
        platformFilter = content.filter(
          (elementFilt) =>
            elementFilt.platform
              .toLowerCase()
              .includes(platform.toLocaleLowerCase()) == true
        ); //filtro por plataforma
      else platformFilter = content; //si no hay plataforma, va todo

      if (filters != "" && filters != "All")
        genreFilter = content.filter(
          (item) => (item.genre.toLowerCase() == filters.toLowerCase()) == true
        ); //filtro por genero
      else genreFilter = content; //si no hay genero, va todo

      if (platformFilter != "" && genreFilter == "")
        contentFiltered = platformFilter;
      else if (platformFilter == "" && genreFilter != "")
        contentFiltered = genreFilter;
      else if (platformFilter == "" && genreFilter == "") contentFiltered = "";
      else {
        contentFiltered = genreFilter.filter(
          (element) => platformFilter.includes(element) == true
        ); //filtro los no repetidos en los dos arrays
      }
    }
  ...
  useEffect(() => {
    ...
    if (
      data.some(
        (element) =>
          element.title.toLocaleLowerCase().startsWith(query) ||
          element.title.toLocaleLowerCase().includes(query) == true
      ) == true
    ) {
    ...
    <>
      {noResults == false ? (
        <section className="resultsGridContainer-search">
          <header className="resultsPagHeader">
            <div className="resultsTitle">
              <div className="TitleContent">
                ...
                <p>
                  <span>{`found ${content.length} results`}</span>
                </p>
              </div>
              <button
          ...
          {!loader && <CardGrid contentGrid={content} use={2} />}
        </section>
      ) : (
